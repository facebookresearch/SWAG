...
model_names = [name for name in hubconf.__dict__
    if name.islower() and not name.startswith("__")
    and callable(hubconf.__dict__[name])]

parser = argparse.ArgumentParser(description='PyTorch ImageNet Training')
parser.add_argument('data', metavar='DIR',
                    help='path to dataset')
parser.add_argument('-m', '--model', metavar='MODEL', default='vit_b16_in1k',
                    choices=model_names,
                    help='model name: ' +
                        ' | '.join(model_names) +
...
parser.add_argument('-b', '--batch-size', default=250, type=int,
                    metavar='N',
                    help='mini-batch size (default: 250), this is the total '
